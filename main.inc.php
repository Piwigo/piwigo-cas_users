...
function casu_init()
{
    ...
    if ( !pwg_get_session_var('noCASU') == 'noCAS' && isset($conf['casu']['casu_host'])) 
    ...
    // if (script_basename() == 'identification' && !pwg_get_session_var('noCASU') == 'noCAS' && isset($conf['casu']['casu_host'])) 
    {
        ...
        phpCAS::client(CAS_VERSION_2_0, $ccas['casu_host'], (int) $ccas['casu_port'], $ccas['casu_context']);
        //This url needs to be the same as defined in CAS settings
        phpCAS::setFixedServiceURL('http://146.59.237.19/Piwigo/');
        ...
        $cas_user = [ 'id' => phpCAS::getUser() ];
        $cas_user['user'] = $cas_user['id'] ;

        if (phpCAS::hasAttributes()) 
        {
            $cas_user['attributes'] = phpCAS::getAttributes();                  
            if(isset($cas_user['attributes'][$ccas['casu_login']]))
            {
